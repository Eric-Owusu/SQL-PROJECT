...
/* finding the average, minimum and the maximum values in the data */

/*Average of years and donuts eaten */
SELECT AVG(years_old) average_years, AVG(donuts_eaten) average_donuts_eaten FROM winstons_donut_logs;

/* Maximum years old and maximum donuts eaten */
SELECT MAX(years_old) max_years_old, MAX(donuts_eaten) max_donuts_eaten FROM winstons_donut_logs;

/* Get the average, max, and min values per category */
SELECT status, AVG(years_old), AVG(donuts_eaten), MAX(donuts_eaten), MIN(donuts_eaten) 
FROM winstons_donut_logs
GROUP BY status;

/* Grouping by year range and leaving the group that does not exist yet */
SELECT 
  CASE 
    WHEN years_old BETWEEN 0 AND 1 THEN '0-1 years'
    WHEN years_old BETWEEN 1 AND 2 THEN '1-2 years'
    WHEN years_old BETWEEN 2 AND 4 THEN '2-4 years'
    ELSE 'Unknown'
  END AS age_group,
  COUNT(*) as count
FROM winstons_donut_logs
GROUP BY age_group;
 ...
 /* Fetch records older than 2 years old and ate more than 20 donuts: */
SELECT * FROM winstons_donut_logs
WHERE years_old > 2 AND donuts_eaten > 20;

/* Group by status and show records where the average number of donuts_eaten is greater than 30.*/
SELECT status, AVG(donuts_eaten)
FROM winstons_donut_logs
GROUP BY status
HAVING AVG(donuts_eaten) > 30;
